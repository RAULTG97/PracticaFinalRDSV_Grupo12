...
#sudo docker exec -it $VNF1 curl -X POST -d '{"port_name": "vxlan1", "type": "linux-htb", "max_rate": "6000000", "queues": [{"max_rate": "2000000"}, {"min_rate": "2000000"}]}' http://127.0.0.1:8080/qos/queue/0000000000000001
#sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "'$IP1'"}, "actions":{"queue": "1"}}' http://127.0.0.1:8080/qos/rules/0000000000000001
#sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "'$IP2'"}, "actions":{"queue": "0"}}' http://127.0.0.1:8080/qos/rules/0000000000000001
