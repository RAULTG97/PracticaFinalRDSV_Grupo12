...
sudo lxc-attach --clear-env -n $BRGX -- bash -c "curl -X POST -d '{\"match\": {\"nw_src\": \"$IP1\"}, \"actions\":{\"queue\": \"1\"}}' http://127.0.0.1:8080/qos/rules/0000000000000001"
sudo lxc-attach --clear-env -n $BRGX -- bash -c "curl -X POST -d '{\"match\": {\"nw_src\": \"$IP2\"}, \"actions\":{\"queue\": \"0\"}}' http://127.0.0.1:8080/qos/rules/0000000000000001"
